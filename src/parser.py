...
def extrair_despesa(texto: str) -> str:
    ...
    if "frete fixo" in texto_lower or "fixo" in texto_lower:
        return "Frete Fixo"
    if "frete variavel" in texto_lower or "frete variável" in texto_lower or "variavel" in texto_lower or "variável" in texto_lower:
...
def extrair_dados_fatura(texto: str) -> List[Dict[str, str]]:
    texto_limpo = texto.replace("\n", " ").replace("\u200b", " ").strip()

    cnpjs = re.findall(r"\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2}", texto_limpo)
    cnpj_ic = cnpjs[0] if len(cnpjs) > 0 else ""
    cnpj_cliente = cnpjs[1] if len(cnpjs) > 1 else ""

    data_emissao = re.search(r"Emissão\s*(\d{2}/\d{2}/\d{4})", texto_limpo)
    vencimento = re.search(r"Vencimento\s*(\d{2}/\d{2}/\d{4})", texto_limpo)
    icms = re.search(r"ICMS\s*:\s*(\d{1,3}(?:\.\d{3})*,\d{2})", texto_limpo)
    numero_fatura = re.search(r"Fatura\s*:\s*(\d+)", texto_limpo)

    tipo, operacao, filial_detectada = extrair_filial(texto_limpo)
    mes_ano = extrair_mes_ano(texto_limpo)
    despesa = extrair_despesa(texto_limpo)
    ...
    faturas_encontradas = re.findall(padrao_faturas, texto_limpo)
    ...
    if not faturas_encontradas:
        resultados.append({
            ...
            "Nº Fatura": numero_fatura.group(1) if numero_fatura else "",
